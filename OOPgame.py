...
class Player:
    ...
    def attack(self, target):
        ...
        time.sleep(1)
        print(
            f"{target.name} Health Points: {GREEN}{target.health}{RESET}/{GREEN}{target.maxhealth}{RESET}"
        )
        print()
    ...
    def defense(self):
        ...
        time.sleep(1)
        print(
            f"Total Health Points now: {GREEN}{self.health}{RESET}/{GREEN}{self.maxhealth}{RESET}"
        )
        print()
...
class Enemy:
    ...
    def attack(self, target):
        ...
        time.sleep(1)
        print(
            f"{BLUE}{target.name}{RESET} Health Points: {GREEN}{target.health}{RESET}/{GREEN}{target.maxhealth}{RESET}"
        )
        print()
...
def menu(player):
    while True:
        ...
        if opponent_choice in ["mahoraga", "3"]:
            mahoraga_fight(player)


def gojo_fight(player):
    ...
    print(" ")
    type_text(
        f"'Suddenly, {enemy_name} stands before {player}. A smirk appears.'", delay=0.1
    )
    time.sleep(1)
    loading_animation([".", ".", "."])
    ...
    type_text(
        f"Crimson Valor: Who doesn't know {enemy_name}? Behind that blindfold, never underestimate him, {player}. He can heal up to {GREEN}300 HP{RESET}",
        delay=0.01,
    )
    type_text(
        f"Use defense to heal yourself, and attack when you get the chance!", delay=0.01
    )
    ...
    while True:
        ...
        if player1.health == 0:
            ...
            options_after_loss = {"1": "Try again", "2": "Main Menu", "3": "Leave"}
        ...
        # PLAYER PILIH
        player_options = {"1": "Attack", "2": "Defense"}
        ...
        if enemy.health == 0:
            ...
            time.sleep(2)
            type_text(
                f"Crimson Valor: Congratulations {player}! You defeated {enemy_name}!"
            )
            time.sleep(2)
            ...
            elif player_choice in ["2", "fight gojo", "two"]:
                type_text(f"Good luck fighting again, {player}!")
                gojo_fight(player)
                break
        ...
        if enemy_choice == enemy_defense:
            if random.choices([True, False], weights=[9, 7])[0]:
                gojo_def = random.choice(gojo_dialog_defense)
                dual_text(*gojo_def, delay=0.05)

        enemy_choice()
        loading_animation(["=>  ", "=>  ", "=>  "])


def sukuna_fight(player):
    ...
    print(" ")
    type_text(
        f"'{enemy_name} approaches. His gaze is sharp towards {player}'", delay=0.1
    )
    time.sleep(1)
    loading_animation([".", ".", "."])
    ...
    type_text(
        f"Crimson Valor: Be careful {player}! {enemy_name} has regeneration abilities exceeding {GREEN}285 HP{RESET}",
        delay=0.01,
    )
...
def mahoraga_fight(player):
    ...
    print(" ")
    type_text(
        f"'Heavy footsteps echo. {enemy_name} slowly appears before {player}.'",
        delay=0.1,
    )
    time.sleep(1)
    time.sleep(2)
    ...
    type_text(
        f"Crimson Valor: {enemy_name} is no weak opponent, {player}! Its power can reach {GREEN}370 HP{RESET}!",
        delay=0.01,
    )
